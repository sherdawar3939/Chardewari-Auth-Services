...
const getRoleDetails = function (req, res) {
  return roleHelper.getRoleDetails(req.conditions)
    .then((data) => {
      generalController.successResponse(res, 'Role Details Fetched successfully.', data, 'role.controller.getRoleDetails')
    }).catch(StandardError, (err) => {
      generalController.errorResponse(res, err, null, 'role.controller.getRoleDetails', SERVER_RESPONSE.VALIDATION_ERROR)
    }).catch((err) => {
      generalController.errorResponse(res, err, 'Please check originalError for details', 'role.controller.getRoleDetails', SERVER_RESPONSE.INTERNAL_SERVER_ERROR)
    })
}
